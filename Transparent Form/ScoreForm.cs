            ...
            else
            {
                ...
                if (!score.checkScore(stdId, cName))
                {
                    ...
                    if (score.insertScore(stdId, cName, scor, desc))
                    {
                        ...
                        MessageBox.Show("เพิ่มคะแนนใหม่", "Add Score", MessageBoxButtons.OK, MessageBoxIcon.Information);

                    }
                    else
                    {
                        MessageBox.Show("ไม่ได้เพิ่มคะแนน", "Add Score", MessageBoxButtons.OK, MessageBoxIcon.Error);
                    }
                }
                else
                {
                    MessageBox.Show("คะแนนสำหรับวิชานี้มีอยู่แล้ว", "Add Score", MessageBoxButtons.OK, MessageBoxIcon.Error);
                }
            }
        }
