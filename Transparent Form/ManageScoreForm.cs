...
namespace Transparent_Form
{
    public partial class ManageScoreForm : Form
    {
        ...
        private void button_Update_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("ต้องการข้อมูลคะแนน", "Field Error", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
            else
            {
                ...
                if (score.updateScore(stdId,cName,scor, desc))
                {
                        ...
                        MessageBox.Show("คะแนนแก้ไขเสร็จสมบูรณ์", "Update Score", MessageBoxButtons.OK, MessageBoxIcon.Information);

                }
                else
                {
                        MessageBox.Show("คะแนนไม่แก้ไข", "Update Score", MessageBoxButtons.OK, MessageBoxIcon.Error);
                }
        ...
        private void button_delete_Click(object sender, EventArgs e)
        {
            ...
            {
                MessageBox.Show("ข้อผิดพลาดในการกรอกข้อมูล - เราต้องการรหัสนักเรียน", "Delete Score", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
            else
            {
                int id = Convert.ToInt32(textBox_stdId.Text);
                if (MessageBox.Show("คุณแน่ใจหรือไม่ว่าต้องการลบคะแนนนี้", "Delete Score", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                {
                    if (score.deleteScore(id))
                    {
                        showScore();
                        MessageBox.Show("คะแนนถูกลบออก", "Delete Score", MessageBoxButtons.OK, MessageBoxIcon.Information);
                        button_clear.PerformClick();
                    }
                }
        ...
        {
            if (DataGridView_score != null && DataGridView_score.CurrentRow != null && DataGridView_score.CurrentRow.Cells.Count >= 6)
            {
